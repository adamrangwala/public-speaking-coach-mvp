...
- boto3 for R2 (S3-compatible)
- ffmpeg-python for media processing
- assemblyai for transcription

System dependencies:
- ffmpeg (installed via Nixpacks/apt in Railway). See [`nixpacks.toml`](nixpacks.toml:5)
...
3) Environment
Create a .env file (see next section). For pure local without R2/transcription you can omit their keys.

4) Initialize DB and folders
```bash
